...
export function NftCard({ tokenId, typeId, name, imageSrc, rawUnitPrice, remainingQuantity }: NftCardProps) {
  ...
  // close when tapping/clicking outside the card
  useEffect(() => {
  ...
  if (typeId !== undefined)
    return (
      <div
        ...
        className={`group w-80 relative h-[30rem] overflow-hidden rounded-3xl bg-surface ring-1 ring-white/10 transition-all duration-200 hover:-translate-y-0.5 hover:ring-[rgba(0,230,118,0.35)] tmt-surface tmt-card-hover ${
      ...
      >
        ...
        <div className="p-4 sm:p-5">
          ...
          <div
            className={`absolute inset-0 flex items-end justify-center bg-black/50 p-4 backdrop-blur-[2px] transition-opacity duration-200 ease-out group-hover:opacity-100 group-hover:pointer-events-auto ${
  ...
  if (tokenId !== undefined) {
    return (
      <div
        ref={cardRef}
        onClick={() => setIsActive(true)}
        className={`group relative overflow-hidden rounded-3xl bg-surface ring-1 ring-white/10 transition-all duration-200 hover:-translate-y-0.5 hover:ring-[rgba(0,230,118,0.35)] tmt-surface tmt-card-hover ${
          isActive ? "-translate-y-0.5 ring-[rgba(0,230,118,0.35)]" : ""
        }`}
      >
        <div aria-hidden className="pointer-events-none absolute inset-x-0 top-0 h-px bg-[linear-gradient(90deg,_transparent,_rgba(0,230,118,0.5),_transparent)] opacity-80" />
